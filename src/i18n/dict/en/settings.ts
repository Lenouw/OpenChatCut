...
export default {
  ...
  '网络代理': 'Network proxy',
  '统一配置服务端访问海外 API 使用的代理地址。': 'One place to set the proxy the server uses to reach APIs abroad.',
  '国内网络访问海外模型（Gemini / OpenAI / Anthropic / Mistral 等）失败时，': 'When a connection to a model abroad (Gemini / OpenAI / Anthropic / Mistral and others) fails,',
  '可在此填写本地代理地址（如 http://127.0.0.1:7890）。': 'enter a local proxy address here (for example http://127.0.0.1:7890).',
  '留空则使用系统环境变量（HTTPS_PROXY / HTTP_PROXY）。': 'Leave it empty to use the system variables (HTTPS_PROXY / HTTP_PROXY).',
  '生效范围：Agent 模型、AI 生成、模型下载、R2 云同步。': 'Applies to: Agent models, AI generation, model downloads and R2 cloud sync.',
  '代理地址': 'Proxy address',
  '例如 http://127.0.0.1:7890': 'For example http://127.0.0.1:7890',
  '本地模型': 'Local models',
  '本地转写、节拍与音乐分析、画面语义搜索。模型按需安装，数据不出本机。': 'Local transcription, beat and music analysis, and search by picture content. Models install on demand and nothing leaves this machine.',
  'API Key（可选）': 'API key (optional)',
  'BytePlus ModelArk 同一个 Key，图生 / 视频生成通用；与「Agent 大脑」的 BytePlus 配置各自独立。': 'One BytePlus ModelArk key covers both image and video generation. It is kept separate from the BytePlus settings under Agent brain.',
  '工程、历史版本与素材的存放位置。默认放在应用数据目录里；': 'Where projects, version history and media are kept. By default in the application data folder;',
  '改到你自己的目录（外置硬盘、同步盘）后，卸载或重装应用都不会动到作品。': 'point it at a folder of your own (external drive, synced folder) and uninstalling or reinstalling never touches your work.',
  '保存时会把现有数据复制到新目录（原目录保留不删），重启应用后生效。': 'Saving copies the existing data to the new folder (the old one is kept) and takes effect after a restart.',
  '桌面端默认把素材存入系统应用数据目录，浏览器开发版默认使用 public/media/uploads/。': 'The desktop app stores media in the system application data folder; the browser development build uses public/media/uploads/.',
  '可选择任意本机目录或外置硬盘；保存后旧目录中的素材会复制到新目录（原文件保留），': 'Choose any local folder or external drive. On save, media in the old folder is copied to the new one (the originals stay),',
  '工程里的素材地址不变，预览与渲染导出都会跟随新目录。': 'media paths inside projects do not change, and preview and export follow the new folder.',
  '未配置时素材只存本机（「本地磁盘」页的目录）。配置后：每次上传同步写入 R2（桶保持私有，': 'Left unset, media stays on this machine only (the folder on the Local disk page). Once set, every upload is also written to R2 (the bucket stays private,',
  '读取经本地服务回源，src 路径不变）；本机缺文件时自动从云端取回。改动即时生效。': 'reads go through the local service and src paths do not change). A file missing locally is fetched back from the cloud. Changes take effect immediately.',
  'R2 控制台建桶 → R2 API Token（Object Read & Write）即可拿到下面四个值。': 'Create a bucket in the R2 console, then an R2 API token (Object Read & Write), to obtain the four values below.',
  '云端隔离 Linux 沙箱，不触碰本机文件。Agent 用它跑 run_code：ffprobe 探测素材时长 / ': 'An isolated Linux sandbox in the cloud that never touches local files. The Agent uses it for run_code: ffprobe to read a clip duration and ',
  '尺寸编码、ffmpeg 转码 / 抽帧 / 加工音视频、执行 node / python 技能脚本，结果回传后': 'its size and codec, ffmpeg to transcode, extract frames or process audio and video, and node or python skill scripts. Results come back and',
  '由本地工具应用到时间线。未配置只影响这些工具，剪辑与预览不受影响。': 'local tools apply them to the timeline. Leaving it unset only disables those tools; editing and preview are unaffected.',
  ...
  '约 743MB': '~743MB',
  '中 / 英': 'Chinese / English',
  '最快最省，适合低配置设备；识别精度一般。': 'Fastest and lightest, suited to modest machines; accuracy is average.',
